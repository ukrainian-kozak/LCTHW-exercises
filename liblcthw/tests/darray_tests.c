...
char *test_create()
{
    ...
    mu_assert(array->contents != NULL, "contents are wrong int darray.");
    mu_assert(array->end == 0, "end isn't at the right spot");
    mu_assert(array->element_size == sizeof(int),
        "element size is wrong.");
    mu_assert(array->max == 100, "Wrong max size in darray.");
...
char *test_get()
{
    ...
    mu_assert(DArray_get(array, 1) == val2, "Wrong the second value.");
...
char *test_remove()
{
    ...
    mu_assert(*val_check == *val1, "Should be gone.");
...
char *test_darray_find()
{
    ...
    int *res = DArray_find(array, &el, (DArray_compare) intcmp);
    mu_assert(*res == el, "Find failed.");
