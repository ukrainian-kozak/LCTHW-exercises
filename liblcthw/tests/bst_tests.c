...
struct tagbstring expect1 = bsStatic("expect 1");
struct tagbstring expect2 = bsStatic("expect 2");
struct tagbstring expect3 = bsStatic("expect 3");
...
char *test_create()
{
    ...
    mu_assert(map != NULL, "Failed to create");
...
char *test_delete()
{
    ...
    bstring result = BSTree_get(map, &test1);
    mu_assert(result == NULL, "Should get NULL.");
    ...
    mu_assert(deleted != NULL, "Got NULL on delete.");
    mu_assert(deleted == &expect3, "Wrong value for test 3.");
    result = BSTree_get(map, &test3);
    mu_assert(result == NULL, "Should delete.");
...
char *test_fuzzing()
{
    ...
    for (i = 0; i < 100; ++i) {
        ...
        mu_assert(value == data[i], "Failed to delete data[i].");
