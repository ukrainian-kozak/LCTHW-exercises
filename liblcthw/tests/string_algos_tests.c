...
struct tagbstring IN_STR = bsStatic(
        "I have ALPHA beta and ALPHA and oranges ALPHA");
...
char *test_find_and_scan()
{
    ...
    mu_assert(find_i > 0, "Failed to find 'APLHA' in test string");

    int scan_i = StringScanner_scan(scan, &ALPHA);
    mu_assert(scan_i > 0, "Failed to fond 'ALPHA' in scanner");
...
char *test_scan_perfomance()
{
    ...
    debug("SCANNER COUNT: %lu, END TIME: %d, OPS: %f",
            find_count, (int)elapsed, (double)find_count / elapsed);
